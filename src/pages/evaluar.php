<div class="main-content h-100" id="evaluar-page">
    ...
    <?php /* if(isset($_SESSION['modulo'][$modulo]['puntaje_evaluacion'])){ ?>
        <div class="ui centered grid h-5 pt-12">
            <div class="twelve wide column mb-6 px-6 py-2 bg-mustard rounded modulo-header page-rating">
                <h2 class="font-size-17">MÓDULO <?= $modulo ?></h2>
            </div>
        </div>
        <div class="ui centered grid h-20 padded">
            <div class="sixteen wide column pt-5 pb-3 pt-1">
                <?php $intento = $_SESSION['modulo'][$modulo]['intentos'] ?>
                <h3><center>Te queda<?= ($intento > 1 ? "n" : "") ?> <?= $intento ?> intento<?= ($intento > 1 ? "s" : "") ?></center></h3>
            </div>
        </div>
        <div class="ui centered grid h-5 pb-12">
            <a href="/universidad/quiz/?modulo=<?= $modulo ?>" class="btn btn-transparent btn-big py-1 px-4 color-white rounded">Iniciar evaluación</a>
        </div>
    <?php }*/ ?>
